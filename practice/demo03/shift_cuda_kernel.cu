...
namespace {
      ...
      const int stride) 
  {
    ...
    if (index < batch*channel*top_height*top_width)
    {
	    ...
	    const int n = index/(channel * top_sp_dim);    // 在哪个batch上   
	    const int idx = index%(channel * top_sp_dim);     // 在batch的哪个位置上，这个不知道具体哪个channel
	    //取余看余数大小，一般就是落在余数的哪里，这里一般是指得落入余数中
		
		const int c_out = idx/top_sp_dim;  // 那个channel                 
	    const int c_in = c_out;                              
	    const int sp_idx = idx%top_sp_dim; // channel中的 二维图像的哪个点（row*dim_row+column）                 
...
at::Tensor shift_cuda_forward2(
  ...
  const dim3 grim_size(input.size(0), input.size(1));
  const dim3 block_size(input.size(2), input.size(3));
